...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // When set, every job renders without an overlay text box (same as the
    // automations "none" mode). Captions are still generated by the AI.
    ...
    const rows = combinations.map(({ reactionId, sourceVideo }, index) => ({
        ...
        overlay_text: noOverlay ? "" : generatedTexts[index].overlayText,
